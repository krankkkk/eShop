...
@Service
public class PriceService implements ProduktserviceInterface {
    ...
    public DiscountPrice getOneDiscountPrice(Long id) {

        return dpRepository.findById(id).orElseThrow(() -> new PriceIdNotFoundException(id));
    }

    public BasicPrice getOneBasicPrice(Long id) {
